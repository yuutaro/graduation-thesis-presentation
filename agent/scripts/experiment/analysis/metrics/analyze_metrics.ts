...
function analyzeFile(filePath: string, limit: number): AnalysisResult {
  ...
  const records = lines.map(line => JSON.parse(line)) as ScenarioRecord[];
  
  const vectors = records.map(r => r.scenario.vector).filter(v => v && v.length > 0);
...
function main() {
  ...
  if (files.length === 0) {
    console.log("Usage: npx tsx analyze_metrics.ts <jsonl_file_1> ... [--limit 100]");
    process.exit(1);
  }
  ...
  for (const filePath of files) {
    ...
    const outputJsonPath = path.join(inputDir, 'similarity_metrics.json');
